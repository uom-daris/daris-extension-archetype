...
# MRI scan
# ================================
proc create_method_MR { ns { action 0 } { fillin 0 } } {
    ...
    :description ${description} \        
        :step < \
            ...
            :subject -part p < \
               :metadata < \
                  :definition -requirement mandatory ${ns}:pssd.anaesthetic \
                  :value < \
        ...
        :step < \
            ...
            :subject -part p < \
               :metadata < \
                  :definition -requirement mandatory ${ns}:pssd.recovery \
                  :value < \
        ...
        :step < \
            ...
            :subject -part p < \
               :metadata < \
                  :definition -requirement mandatory ${ns}:pssd.animal.kill \
                  :value < \
...
proc create_method_Fix { ns { action 0 } } {
    ...
    # Set arguments based on desired action 
    set margs [set_method_update_args $id $action]
    if { $margs == "quit" } {
        return
    }
    # Set Method body
    set args "${margs} \
        ...
        :description ${description} \
            :step < \
                ...
                :subject -part p < \
                   :metadata < \
                       :definition -requirement mandatory ${ns}:pssd.EAE.perfusion  \
                       :value < \
                         :agent < \
            ...
            :step < \
                ...
                :subject -part p <\
                   :metadata < \
                      :definition -requirement mandatory ${ns}:pssd.EAE.optic-nerve.removal  \
                      :value < \
                        :method ${value2a} \
                      > \
                   >\
...
proc create_method_OMM { ns { action 0 } } {    
    ...
    set args "${margs} \
       ...
       :step < \
            ...
            :subject -part p <\
               :metadata < \
                   :definition -requirement mandatory ${ns}:pssd.EAE.optic-nerve.section  \
                   :value <\
        ...
        :step < \
            ...
            :subject -part p <\
               :metadata < \
                   :definition -requirement mandatory ${ns}:pssd.EAE.stain  \
                   :value < \
                     :agent < \
                       :name ${value2a} \
                     >\
        ...
        :step < \
            ...
            :study < \
               :type ${type} \
               :metadata <\
                 :definition -requirement mandatory ${ns}:pssd.EAE.microscopy  \
                 :value <\
...
proc create_method_EMM { ns { action 0 } } {
...
proc create_method_Overall { mrMethodId fixMethodId OMMethodId EMMethodId ns { action 0 } } {
            ...
            :subject <\
               :project <\
                  :public < \
                     ...
                     > \
                     :metadata < \
                        :definition -requirement mandatory ${ns}:pssd.animal.disease \
                        :value < \
                          :disease ${value1} \
                        > \
                     > \
                     :metadata < \
                        :definition -requirement mandatory ${ns}:pssd.animal.genetics \
                        :value < \
                          :strain < \
                     ...
                     > \
                     :metadata < \
                        :definition -requirement optional hfi.pssd.identity \
                     > \
                  > \
               >\
            >\
...
#####################################################################################
proc create_method_EAE { doc_ns { action 0 } } {
    set mrMethodId      [create_method_MR  ${doc_ns} $action]
    set fixMethodId     [create_method_Fix ${doc_ns} $action]
    set OMMethodId      [create_method_OMM ${doc_ns} $action]
    set EMMethodId      [create_method_EMM ${doc_ns} $action]
    set overallMethodId [create_method_Overall $mrMethodId $fixMethodId $OMMethodId $EMMethodId ${doc_ns} $action]
